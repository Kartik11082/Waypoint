...
async def get_clues_for_story(story_id: str) -> dict:
    cached = read_item_cache('clues', story_id)
    if cached:
        return cached

    story = await get_story_by_id(story_id)
    clues = await asyncio.to_thread(generate_clues, story)
    if not clues:
        raise HTTPException(500, 'Failed to generate clues')

    result = {
        'story_id': story_id,
        **{k: clues[k] for k in ['clue1', 'clue2', 'clue3', 'category', 'difficulty']}
    }
...
async def clues_endpoint(story_id: str):
    return await get_clues_for_story(story_id)
